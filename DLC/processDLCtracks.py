...
author: Felix Hol
date: 2021 Nov
content: cut individual DLC tracks into individual dataframes and save (including disconnected tracks labeled as same individual)
'''
...
#### collect all H5 DLC files that need to be processed.
hdfs = glob.glob('/mnt/DATA/biteData/P3/210715_mp4/*bx.h5')

minLen = 4              ### minimum lenght of a trajectory to be included
smoothLenXY = 2         ### averaging window to create smoothed x and y
smoothLenDist = 5       ### averaging window to create smoothed distance
smoothLenBelly = 50     ### averaging window to create smoothed belly width
minEngInc = 1.5         ### minimum increase of abdominal width for mosquito to be scored as engorged
minEngW = 37            ### minimal abdominal width for a mosquito to be scored as engorged
minEngTime = 200        ### minimal time (in frames) in which a mosquito can engorge
maxMinW = 35            ### minimal abdominal width to be scored as 'already engorged'
extraLenEngorge = 250   ### tracking data is truncated this number of frames after a mosquito is scored as engorged
